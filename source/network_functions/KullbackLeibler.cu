    ...
    ensemble.foreach(
        ...
        [=] __device__ __host__ (
            ...
            SINGLE {
                ...
                generic_atomicAdd(this_.prob_ratio, prob_ratio * weight);
                generic_atomicAdd(this_.mean_deviation, prob_ratio * weight * (log_psi_prime - log_psi));
                deviation = log_psi_prime - log_psi - *this_.last_mean_deviation;

                deviation2 = abs2(deviation);
                if(deviation2 > threshold2) {
                    generic_atomicAdd(this_.deviation, prob_ratio * weight * deviation);
                    generic_atomicAdd(this_.deviation2, prob_ratio * weight * deviation2);
                }
            }
            SYNC;
            ...
            if(compute_gradient) {
                psi_prime_kernel.foreach_O_k(
                    ...
                    [&](const unsigned int k, const complex_t& O_k) {
                        generic_atomicAdd(
                            &this_.O_k[k],
                            prob_ratio * weight * O_k
                        );
                        if(deviation2 > threshold2) {
                            generic_atomicAdd(
                                &this_.deviation_O_k_conj[k],
                                prob_ratio * weight * deviation * conj(O_k)
                            );
                        }
                        ...
                        if(noise) {
                            ...
                            if(deviation2 > threshold2) {
                                generic_atomicAdd(
                                    ...
                                    prob_ratio * weight * deviation2 * O_k2
                                );
                                generic_atomicAdd(
                                    &this_.deviation_O_k[k],
                                    prob_ratio * weight * deviation * O_k
                                );
                                generic_atomicAdd(
                                    &this_.deviation_O_k2[k],
                                    prob_ratio * weight * deviation * O_k2
                                );
                                generic_atomicAdd(
                                    &this_.deviation2_O_k[k],
                                    prob_ratio * weight * deviation2 * O_k
                                );

                            }
                            generic_atomicAdd(
                                &this_.O_k2[k],
                                prob_ratio * weight * O_k2
                            );
                        }
                    }
                );
