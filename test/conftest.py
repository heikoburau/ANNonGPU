...
def pytest_generate_tests(metafunc):
    ...
    if 'psi_cnn' in metafunc.fixturenames:
        psi_list = [
            # lambda gpu: new_convolutional_network([3], [(2, [3])], noise=1e-1, gpu=gpu),
            # lambda gpu: new_convolutional_network([3, 3], [(2, [3, 3])], noise=1e-1, gpu=gpu),
            # lambda gpu: new_convolutional_network([4, 4], [(5, [4, 4])], noise=1e-2, gpu=gpu),
            lambda gpu: new_convolutional_network(
                [2, 4, 4],
                [(2, [2, 4, 4]), (4, [2, 2, 2])],
                noise=1e-2, gpu=gpu
            ),
